...
public class BusinessAnalyticsRestController {
        ...
        public ResponseEntity<Map<String, Object>> getBusinessSummary(@PathVariable Long businessUserId) {
                ...
                BigDecimal totalReceived = transactionService.getTransactionsByUserId(businessUserId).stream()
                                ...
                                .filter(t -> t.getType() == TransactionType.SEND
                                                || t.getType() == TransactionType.PAYMENT
                                                || t.getType() == TransactionType.ADD_FUNDS)
                                .map(t -> t.getAmount())
                ...
                BigDecimal outstandingInvoiceAmount = invoiceService.getInvoicesByBusinessUserId(businessUserId)
                                .stream()
                                .filter(i -> i.getStatus() == InvoiceStatus.SENT
                                                || i.getStatus() == InvoiceStatus.OVERDUE)
