...
public class ITransactionServiceImpl implements ITransactionService {
        ...
        @Override
        @Transactional(noRollbackFor = { InsufficientFundsException.class, BadRequestException.class,
                        InvalidCredentialsException.class, InvalidTransactionPinException.class })
        @Caching(evict = {
        ...
        public TransactionDTO sendMoney(Long senderId, Long recipientId, BigDecimal amount, String note, String pin) {
                log.info("Initiating sendMoney from sender ID {} to recipient ID {} for amount {}", senderId,
                                recipientId,
                                amount);
                ...
                if (sender.getTransactionPin() == null) {
                        ...
                        throw new InvalidTransactionPinException(
                                        "You have not set a transaction PIN. Please set one in your Profile settings before sending money.");
                }
                ...
                Wallet senderWallet = walletRepository.findByUserId(senderId)
                                .orElseGet(() -> {
                                        log.info("Sender wallet not found for ID {}. Creating one...", senderId);
                                        com.rev.app.dto.WalletDTO newWalletDto = walletService.createWallet(senderId);
                                        return transactionMapper.toWalletEntity(newWalletDto);
                                });
                Wallet recipientWallet = walletRepository.findByUserId(recipientId)
                                .orElseGet(() -> {
                                        log.info("Recipient wallet not found for ID {}. Creating one...", recipientId);
                                        com.rev.app.dto.WalletDTO newWalletDto = walletService
                                                        .createWallet(recipientId);
                                        return transactionMapper.toWalletEntity(newWalletDto);
                                });

                if (senderWallet.getBalance().compareTo(amount) < 0) {
                        log.warn("Transfer failed: Insufficient funds. Sender balance: {}, Transfer amount: {}",
                        ...
                        notificationService.createNotification(senderId,
                                        "Payment failed: Insufficient funds to send INR " + amount + ".",
                ...
                notificationService.createNotification(sender.getId(),
                                "You sent INR " + amount + " to " + recipient.getFullName()
        ...
        @Override
        public List<TransactionDTO> filterTransactions(Long userId, TransactionType type, String startDate,
                        String endDate,
                        BigDecimal minAmount, BigDecimal maxAmount, String status) {
